-- Allow public supplier/buyer signup to create their own organization.
-- This is implemented in the auth/users AFTER INSERT trigger so it works
-- even when the user must confirm email (session may not exist immediately).

BEGIN;

-- Prototype convenience flag (derived from verification_status).
...
BEGIN
  ...
  verification_status_value := CASE
    WHEN meta_is_verified IN ('true', '1', 'yes') THEN 'verified'::public.org_verification_status
    ELSE 'pending'::public.org_verification_status
  END;
